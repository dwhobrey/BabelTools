//  HardwareKeys.cpp: based on diskid32.cpp, 01/29/08, Lynn McGuire  written with many contributions from others.
...
int ReadPhysicalDriveInNTWithAdminRights (void)
{
	...
	for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
	{
		...
		sprintf_s (driveName, (size_t)256,"\\\\.\\PhysicalDrive%d", drive);
...
int ReadDrivePortsInWin9X (void)
{
	...
	if (VxDHandle != INVALID_HANDLE_VALUE)
	{
        ...
        DWORD err = GetLastError();
        char info[128];
        sprintf(info, "ERROR (%lx): Could not open IDE21201.VXD file.", err);
        MessageBoxA(NULL, info,
            TITLE, MB_ICONSTOP);
    }
...
// Fetches PC key values:
// ComputerId = DWORD.
// Primary MAC = DWORD.
// Returns Primary HD SN as a ASCII string.
char* GetHardwareKeys(DWORD * computerId, DWORD* mac) {
